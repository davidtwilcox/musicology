...
- **Data**: `CHROMATIC_TONES` (12 tones, C–B using sharps). `SCALE_TYPES` (11 types: 4 common scales + 7 modes). `SCALE_INTERVALS` maps each type to its semitone interval pattern from the root. `ENHARMONIC_FLAT` remaps D♯/G♯/A♯ to E♭/A♭/B♭ when needed.
...
- **UI building**: `buildScalesTab()` creates `.scale-block` buttons for each tonic and scale type, inserting a `.scales-type-separator` labeled "Modes" before Ionian (index 4 in `SCALE_TYPES`).
