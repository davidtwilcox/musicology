...
- **Data**: `CHROMATIC_TONES` (12 tones, C–B using sharps). `SCALE_TYPES` (16 types: 4 common scales + 7 modes + 5 other). `SCALE_INTERVALS` maps each type to its semitone interval pattern from the root. `ENHARMONIC_FLAT` remaps D♯/G♯/A♯ to E♭/A♭/B♭ when needed. Non-heptatonic scales are supported: whole tone has 6 notes, diminished has 8.
