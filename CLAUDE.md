...
- **Scale computation**: `computeScale(tonic, intervals, letterOffsets)` derives note names algorithmically — uses `letterOffsets` (if provided) to assign the correct diatonic letter per degree and adds ♯/♭ to match the target semitone. For heptatonic scales `letterOffsets` is omitted and consecutive letters are assumed. `SCALE_LETTER_OFFSETS` provides explicit letter offset arrays for pentatonic scales, which skip letters (major pentatonic skips the 4th and 7th degrees; minor pentatonic skips the 2nd and 6th). `hasDoubleAccidental(tonic, intervals, letterOffsets)` detects when a tonic+scale combination would require double sharps. `computeScaleNotes(tonic, scaleType)` calls `hasDoubleAccidental` and automatically uses the enharmonic flat tonic when double sharps would occur.
...
- **Staff rendering**: `renderStaffSvg` uses letter-based staff positioning (`LETTER_IDX` + octave tracking) rather than incrementing by 1 per note. This correctly handles non-heptatonic scales where the octave-return note would land on the wrong staff line with a naive `startPos + i` approach.
