...
- **Chord Borrowing section**: Contains three sub-sections — Parallel, Modal Interchange, and Secondary Dominants. Each uses a `.borrowing-table` (7-column layout matching diatonic degrees) with clickable chord cells. Chord selections across all sub-sections and the Diatonic Chords table are mutually exclusive, managed by `clearAllChordSelections()`.
  ...
  - **Modal Interchange**: `getModalChords(tonic, modeName)` computes triads for each mode by building the scale via `computeScaleNotes`, then determining chord quality from semitone intervals (`noteToSemitone`). Shows 7 mode rows; the current mode (Ionian for major, Aeolian for minor) is labeled "Current" and moved to the top.
  - **Secondary Dominants**: `computeSecondaryDominant(targetRoot)` finds the perfect 5th above the target root (letter-based computation for correct spelling), builds a dominant 7th chord via `computeScale` with `dom7Intervals [0,4,7,10]` and `dom7Offsets [0,2,4,6]`, and remaps enharmonically when `hasDoubleAccidental` detects double sharps. Shows three rows: **Current** (diatonic chord), **V7 /** (secondary dominant dom7), and **♭II7 /** (tritone substitution). The numeral stored in `data-secdom-num` for V7/ cells is `"V7/x"` so the Chord Information panel title correctly reads "V7/I", "V7/ii", etc. `computeTritoneSubstitution(secDomRoot)` finds the note 6 semitones from the secondary dominant (spelled using +4 letter steps from the sec dom's letter — the diminished 5th direction, giving ♭II of the original target) and builds a dominant 7th on that root. All three rows share the same `data-secdom-*` click handler, which dispatches to `showSecDomCurrentTones`, `showSecDomChordTones`, or `showTritoneSubChordTones` based on `data-secdom-type`.
- **Chord Information panel**: `#chord-info-panel` is a separate panel to the right of `#info-panel`, hidden until a chord is clicked. All chord-tone displays (diatonic, borrowed, modal, secondary dominant) render into this single panel via `renderChordToneDisplay(title, tones, color)`. When shown, `main` max-width expands from 1100px to 1700px (set via inline style); when hidden (chord deselected, key changed, or selection cleared), max-width resets. Both panels use `flex: 1` so they share space equally and have matching widths. The panel has `position: sticky; top: 2rem` to stay visible while scrolling the info panel.

### Chords tab

- **Layout**: Mirrors the Scales tab — three selector columns (Root, Type, Inversion) plus a `#chord-result` panel that appears when both root and type are selected. Uses the same `.scales-page` / `.scales-layout` / `.scales-col` / `.scale-block` CSS classes.
- **Data**: `CHORD_TYPES` (13 types). `CHORD_TYPE_INTERVALS` maps each type to its semitone interval array. `CHORD_TYPE_LETTER_OFFSETS` maps each type to diatonic letter offsets passed to `computeScale` — triads and 7th chords use `[0,2,4]` / `[0,2,4,6]`; sus2 uses `[0,1,4]`, sus4 `[0,3,4]`, 7sus4 `[0,3,4,6]`, Major 6th `[0,2,4,5]`, 5th (power chord) `[0,4]`. `CHORD_TYPE_INTERVAL_NAMES` maps each type to human-readable interval names. `INVERSIONS = ['Root','First','Second','Third']`.
- **Chord computation**: `renderChordResult(root, chordType)` reuses `computeScale` and applies the same `ENHARMONIC_FLAT` / `hasDoubleAccidental` remapping as the Scales tab. Displays a note-sequence line, a tones table (Note / Interval), staff SVG, and piano SVG.
- **Inversions**: `selectedInversion` ('Root' / 'First' / 'Second' / 'Third', default 'Root'). Each inversion rotates the notes array left by its index (1–3); `intervalNames` rotates in parallel so the tones table stays in sync. The heading appends " — First/Second/Third Inversion" when not root position. Disabled-inversion rules: First and Second are disabled for the 5th (power chord, 2 notes); Third is disabled for all non-tetrads (`noteCount < 4`). `updateChordBlocks()` computes `noteCount = CHORD_TYPE_INTERVALS[selectedChordType].length` and resets `selectedInversion` to 'Root' when the active inversion becomes disabled. Disabled buttons use `button.disabled = true` with `.scale-block:disabled { opacity: 0.35; pointer-events: none; }`.
- **State**: `selectedChordRoot`, `selectedChordType`, `selectedInversion`. `buildChordsTab()` builds all three columns; `updateChordBlocks()` syncs button active/disabled states and re-renders the result panel.
