...
- **Inversions**: `selectedInversion` ('Root' / 'First' / 'Second', default 'Root'). First inversion rotates the notes array left by 1 (`notes.slice(1).concat(notes.slice(0,1))`); Second rotates by 2. The `intervalNames` array is rotated in parallel so the tones table stays in sync with the voicing. The heading appends " — First Inversion" / " — Second Inversion" when not in root position. The 5th (power chord) disables First and Second buttons and forces Root; all other types support all three inversions. Disabled buttons use `button.disabled = true` with `.scale-block:disabled { opacity: 0.35; pointer-events: none; }`.
